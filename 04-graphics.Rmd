...
```{r, fig.margin = TRUE, fig.cap="Examples of scatterplots made for the Iris data. A) A default scatter plot showing the relationship between petal length and width. B) The same as (A) but with a correlation line added."}
...
```{r, fig.cap="A bevy of density graphs option based on the iris data. A) A lone density graph. B) A histogram with a density graph overlay. C) A density graph accompanied by a rug plot. D) A ridge plot."}
