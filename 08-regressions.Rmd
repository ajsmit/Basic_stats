...
#            error_fix = if_else((waiting >= mean(waiting)), -(eruptions_error), eruptions_error))
# 
#   # Set intercept value
#   intercept <- round(mean(faithful_pred$eruptions)-mean(faithful_pred$eruptions_pred), 0)
# 
#   # Sum of Squares (SS)
#   total_ss_val <- round(sum((faithful_pred$eruptions-mean(faithful_pred$eruptions))^2), 0)
#   residual_ss_val <- round(sum(faithful_pred$eruptions_error^2), 0)
#   regression_ss_val <- round(sum((faithful_pred$eruptions_fit-mean(faithful_pred$eruptions))^2), 0)
# 
#   # Mean square (MS)
...
#     geom_rect(aes(xmin = waiting, xmax = waiting+abs(eruptions_error),
#                   ymin = eruptions, ymax = eruptions_fit,
#                   fill = error_fix, alpha = alpha_range)) +
#     geom_segment(size = 0.2, alpha = 0.7, aes(xend = waiting, y = eruptions_fit, yend = eruptions)) +
#     geom_point(shape = 21) +
#     geom_line(aes(y = eruptions_fit)) +
#     annotate("text", x = -20, y = 625, label = paste0("slope == ", i, "~(min/min)"), parse = TRUE, hjust = 0) +
...
#     annotate("text", x = -20, y = 500, label = paste0("d.f. == ", nrow(faithful_pred)-2), parse = TRUE, hjust = 0) +
...
# system.time(saveVideo(animate_lm(), interval = 0.01, ani.width = 600, video.name = "lm_rotate.mp4")) #~553 seconds

# knitr::include_graphics("figures/lm_rotate.mp4")
```

```{r, echo=FALSE}
# Does not eem to work
# knitr::include_url("https://raw.githubusercontent.com/ajsmit/Basic_stats/master/figures/lm_rotate.mp4", height = "480px")
```
