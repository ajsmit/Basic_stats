...
#   faithful_pred <- faithful %>%
#     mutate(eruptions_pred = waiting*i, 
...
#     # Interesting... these following two lines produce the same results...
#   # rss_val <- round(sum(faithful_pred$eruptions_error^2), 4)
#   # rss_val <- round(sum(faithful_pred$eruptions_error^2)-((sum(faithful_pred$eruptions_error)^2)/nrow(faithful_pred)), 4)
#   # This is a competing equation... seems more accurate
#   rss_val <- round(sum((faithful_pred$eruptions_fit-mean(faithful_pred$eruptions))^2)/(nrow(faithful_pred)), 4)
#   se_val <- sqrt(var(faithful_pred$eruptions_fit)/nrow(faithful_pred))
#   f_val <- (rse_val/1)/(rss_val/(nrow(faithful_pred)-2))
#   # test <- qf(0.95, 1, 270)
#   p_val <- 1-pf(f_val, 1, (nrow(faithful_pred)-2))
#   r2_val <- round(cor(faithful_pred$waiting, faithful_pred$eruptions)^2, 2)
#   
...
#     geom_segment(data = bars_decrease_right, size = 1, alpha = 0.7,
#                  aes(xend = waiting, y = eruptions_fit, yend = eruptions, colour = eruptions_error)) +
#     geom_segment(data = bars_increase_right, size = 1, alpha = 0.7,
#                  aes(xend = waiting, y = eruptions_fit, yend = eruptions, colour = eruptions_error)) +
#     geom_segment(data = bars_decrease_left, size = 1, alpha = 0.7,
#                  aes(xend = waiting, y = eruptions_fit, yend = eruptions, colour = eruptions_error)) +
#     geom_segment(data = bars_increase_left, size = 1, alpha = 0.7,
#                  aes(xend = waiting, y = eruptions_fit, yend = eruptions, colour = eruptions_error)) +
#     geom_point() +
#     geom_line(aes(y = eruptions_fit)) +
#     annotate("text", x = 45, y = 5.25, label = paste0("slope == ", i, "~(min/min)"), parse = TRUE, hjust = 0) +
#     annotate("text", x = 45, y = 5.00, label = paste0("intercept == ", intercept), parse = TRUE, hjust = 0) +
#     annotate("text", x = 45, y = 4.75, label = paste0("italic(r)^2 == ", r2_val), parse = TRUE, hjust = 0) +
#     annotate("text", x = 45, y = 4.50, label = paste0("italic(p)", p_val_print), parse = TRUE, hjust = 0) +
#     annotate("text", x = 45, y = 4.25, label = paste0("RSE == ", rse_val), parse = TRUE, hjust = 0) +
#     annotate("text", x = 45, y = 4.00, label = paste0("RSS == ", rss_val), parse = TRUE, hjust = 0) +
#     # stat_smooth(method = "lm", colour = "salmon") +
#     scale_colour_gradient2(low = "red", mid = "white", high = "blue", guide = FALSE) +
#     coord_cartesian(ylim = c(1.2, 5.6)) +
...
# animate_lm <- function() {
#   lapply(seq(0.00, 0.15, by = 0.0001), function(i) {
#     lm_fig(i)
#   })
# }
...
# system.time(saveVideo(animate_lm(), interval = 0.1, ani.width = 600, video.name = "lm_rotate.mp4")) ~553 seconds
