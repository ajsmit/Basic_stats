...
# And the sum of squares must of course be included with the figure, and maybe also the p-value and the SE of the slope estimate.

# I think for this one we are perhaps permitted to have a slightly larger movie--say 10Mb? Then it looks nicer.

# Can the animation not be saved as an AVI or MPEG, which may be smaller?
...
lm_fig <- function(i){
  ...
  # Create random data
  set.seed(666)
  random_dat <- data.frame(eruptions = rnorm(50, 10, 1),
                           waiting = rnorm(50, 10, 1),
                           row_seq = seq(1, 50))
  random_dat <- random_dat %>% 
    mutate(eruptions = eruptions * row_seq, 
           waiting = waiting * row_seq)

  # Calculate fitted values
  faithful_pred <- random_dat %>%
    mutate(eruptions_pred = waiting*slope,
           eruptions_fit = eruptions_pred + mean(eruptions) - mean(eruptions_pred),
           eruptions_error = eruptions_fit - eruptions,
           alpha_range = 1-(abs(eruptions_error)/max(abs(eruptions_error))))

  # Set intercept value
  intercept <- round(mean(faithful_pred$eruptions)-mean(faithful_pred$eruptions_pred),2)

  # Pull out increasing and decreasing bars
  bars_decrease_right <- faithful_pred %>%
    filter(waiting >= mean(waiting), eruptions_error <= 0) %>%
    mutate(eruptions_error = abs(eruptions_error))
  bars_increase_right <- faithful_pred %>%
    filter(waiting >= mean(waiting), eruptions_error >= 0) %>%
    mutate(eruptions_error = -eruptions_error)
  bars_decrease_left <- faithful_pred %>%
    filter(waiting <= mean(waiting), eruptions_error >= 0)
  bars_increase_left <- faithful_pred %>%
    filter(waiting <= mean(waiting), eruptions_error <= 0)

  # Calculate stats for plotting
  rse_val <- round(sqrt(sum(faithful_pred$eruptions_error^2))/(nrow(faithful_pred)), 4)
  se_val <- sqrt(var(faithful_pred$eruptions_fit)/nrow(faithful_pred))
  
  # Sum of Squares (SS)
  total_ss_val <- round(sum((faithful_pred$eruptions-mean(faithful_pred$eruptions))^2), 2)
  residual_ss_val <- round(sum(faithful_pred$eruptions_error^2), 2)
  regression_ss_val <- round(sum((faithful_pred$eruptions_fit-mean(faithful_pred$eruptions))^2), 2)
  
  # Mean square (MS)
  regression_ms_val <- regression_ss_val/1
  residual_ms_val <- residual_ss_val/(nrow(faithful_pred)-2)
  
  # Final stats
  f_val <- round(regression_ms_val/residual_ms_val, 2)
  test <- qf(0.95, 1, 270)
  p_val <- 1-pf(f_val, 1, (nrow(faithful_pred)-2))
  ...
  poly_1 <- faithful_pred %>% 
    filter(waiting >= min(waiting))
  poly_2 <- faithful_pred %>% 
    filter(waiting <= min(waiting))

  # Create the figure
  lmr <- ggplot(data = faithful_pred, aes(x = waiting, y = eruptions)) +
    # geom_rect(data = poly_1, alpha = 0.6, fill = "grey60",
    #           aes(xmin = waiting, xmax = waiting+abs(eruptions_error), 
    #               ymin = eruptions, ymax = eruptions_fit, colour = eruptions_error)) +
    # geom_rect(data = poly_2, alpha = 0.6, fill = "grey60",
    #           aes(xmin = waiting, xmax = waiting+abs(eruptions_error), 
    #               ymin = eruptions, ymax = eruptions_fit, colour = -eruptions_error)) +
    geom_rect(data = bars_decrease_right,
              aes(xmin = waiting, xmax = waiting+abs(eruptions_error),
                  ymin = eruptions, ymax = eruptions_fit, 
                  fill = eruptions_error, alpha = alpha_range)) +
    geom_segment(size = 0.2, alpha = 0.7, aes(xend = waiting, y = eruptions_fit, yend = eruptions)) +
    # geom_segment(data = bars_increase_right, size = 0.2, alpha = 0.7,
    #              aes(xend = waiting, y = eruptions_fit, yend = eruptions)) +
    # geom_segment(data = bars_decrease_left, size = 0.2, alpha = 0.7,
    #              aes(xend = waiting, y = eruptions_fit, yend = eruptions)) +
    # geom_segment(data = bars_increase_left, size = 0.2, alpha = 0.7,
    #              aes(xend = waiting, y = eruptions_fit, yend = eruptions)) +
    # colour = eruptions_error
    geom_point(shape = 21) +
    geom_line(aes(y = eruptions_fit)) +
    # annotate("text", x = 42, y = 5.5, label = paste0("slope == ", i, "~(min/min)"), parse = TRUE, hjust = 0) +
    # annotate("text", x = 42, y = 5.25, label = paste0("intercept == ", intercept), parse = TRUE, hjust = 0) +
    # annotate("text", x = 42, y = 5.00, label = paste0("italic(r)^2 == ", r2_val), parse = TRUE, hjust = 0) +
    # annotate("text", x = 42, y = 4.75, label = paste0("residual~SS == ", residual_ss_val), parse = TRUE, hjust = 0) +
    # annotate("text", x = 42, y = 4.50, label = paste0("regression~SS == ", regression_ss_val), parse = TRUE, hjust = 0) +
    # annotate("text", x = 42, y = 4.25, label = paste0("d.f. == ", nrow(faithful_pred)-2), parse = TRUE, hjust = 0) +
    # annotate("text", x = 42, y = 4.00, label = paste0("F == ", f_val), parse = TRUE, hjust = 0) +
    # annotate("text", x = 42, y = 3.75, label = paste0("italic(p)", p_val_print), parse = TRUE, hjust = 0) +
    # stat_smooth(method = "lm", colour = "salmon") +
    # scale_colour_gradient2(low = "red", mid = "white", high = "blue", guide = FALSE) +
    scale_fill_gradient2(low = "red", mid = "white", high = "blue", guide = FALSE) +
    scale_alpha(guide = FALSE) +
    # coord_cartesian(ylim = c(1.15, 5.65), xlim = c(40, 100)) +
    coord_equal(xlim = c(-50, 650), ylim = c(-50, 650)) +
    labs(title = "Rotating linear fit",
         # subtitle = "Rotating linear fit",
         x = "X",
         y = "Y") +
    theme_dark()
  print(lmr)
...
animate_lm <- function() {
  # lapply(c(seq(0.00, 0.09, by = 0.001), rep(0.08, 1000)), function(i) {
  lapply(c(seq(0.00, 1.5, by = 0.01), seq(1.4, 1.0, by = -0.01), rep(1.0, 10)), function(i) {
    lm_fig(i)
  })
}
...
system.time(saveVideo(animate_lm(), interval = 0.5, ani.width = 600, video.name = "lm_rotate.mp4")) #~553 seconds

# knitr::include_graphics("figures/lm_rotate.mp4")
```
