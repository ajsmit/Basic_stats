...
lm_fig <- function(i){
  ...
  #                        sample = "A")
  faithful_fake <- faithful %>%
    mutate(eruptions_fake = waiting*i) %>% 
    mutate(eruptions_corrected = eruptions_fake + mean(eruptions)-mean(eruptions_fake))
  
  rse_val <- sqrt(sum(faithful_fake$eruptions_corrected-faithful_fake$eruptions)^2)/(nrow(faithful_fake)-2)
  
  lmr <- ggplot(data = faithful_fake, aes(x = waiting, y = eruptions)) +
    geom_point() +
    geom_segment(aes(x = waiting, xend = waiting, 
                 y = eruptions_corrected,  yend = eruptions), 
                 size = 1, alpha = 0.7, colour = "red", linetype = "dotted") +
    annotate("text", x = 45, y = 5, label = paste0("slope == ", i, "~(min/min)"), parse = TRUE, hjust = 0) +
    annotate("text", x = 45, y = 4.75, label = paste0("RSE == ", rse_val), parse = TRUE, hjust = 0) +
    # stat_smooth(method = "lm", colour = "salmon") +
    geom_line(aes(y = eruptions_corrected)) +
...
animate_lm <- function() {
  lapply(seq(0.00, 0.1, by = 0.01), function(i) {
    lm_fig(i)
  })
}

# Note that this spits out in the main directory and is manually moved to the figures folder
system.time(saveGIF(animate_lm(), interval = 1, ani.width = 600, movie.name = "lm_rotate.gif")) ## ~12 seconds

knitr::include_graphics("figures/lm_rotate.gif")
```
